...
for _ in range(1000):
    for bid in range(len(train_x.shape[0]/300)):
        x1,y1=train_x[bid*100:(bid+1)*100],y_train[bid*100:(bid+1)*100]
        sess.run(train_step,feed_dict={X:x1, Y:y1,keep_prob: 0.5})
    if _%10==0:
        [a,s] =sess.run([accuracy, summ],feed_dict={X:private_test_x, Y:y_private,keep_prob: 0.5}) 
        writer.add_summary(s,_)
        print("steps %d accuracy %g" %(_,a))
    

save_path = saver.save(sess, "graphTrain/My_first_model_train_tf2")
print(sess.run(accuracy,feed_dict={X:public_test_x,Y:y_public}))
